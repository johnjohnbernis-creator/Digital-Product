...
def validate_planisware(planisware_feature: str, planisware_number: Any) -> Optional"""Require JJMD-####### when Planisware Feature is Yes."""
...
def distinct_values(col: str) -> Listq = f'SELECT DISTINCT "{col}" AS v FROM "{TABLE}" WHERE "{col}" IS NOT NULL AND TRIM("{col}") <> \'\' ORDER BY v'
    with conn() as c:
